...
### Fisher Test
```{r}
fisher_tab<-colnames(patMeta_cl) %>% 
  as_data_frame() 
...
fisher_test<-patMeta_cl %>% 
  select(Cluster, i)

colnames(fisher_test)[2]<-"Gen_Alt" 
  
fisher_test<-  group_by(fisher_test, Cluster, Gen_Alt) %>% 
  filter(!is.na(Gen_Alt)) %>% 
  count() 

if(nrow(fisher_test)>4){
  
 fisher_test<- fisher_test %>% 
    ungroup() %>% 
  pivot_wider(names_from = Gen_Alt, values_from = n) %>% 
...
fisher_tab[i,2]<-fisher_test$p.value
}
...
fisher_test<-patMeta_cl %>% 
  ...
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = IGHV.status, values_from = n) %>% 
  column_to_rownames("Cluster") %>% 
  as.matrix() %>% 
  stats::fisher.test()

fisher_tab[which(fisher_tab$value=="IGHV.status"),2]<-fisher_test$p.value



fisher_test<-patMeta_cl %>% 
  select(Cluster, Methylation_Cluster) %>% 
  group_by( Cluster, Methylation_Cluster) %>% 
  filter(!is.na(Methylation_Cluster)) %>% 
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = Methylation_Cluster, values_from = n) %>% 
  column_to_rownames("Cluster") %>% 
  as.matrix() %>% 
  stats::fisher.test(simulate.p.value=TRUE)

fisher_tab[which(fisher_tab$value=="Methylation_Cluster"),2]<-fisher_test$p.value

fisher_tab %>% 
  rename(Gen_Alt=value, fisher_p_value=`...2`) %>% 
...
```{r}
patMeta_cl %>% 
  select(Cluster, del11q) %>% 
  group_by( Cluster, del11q) %>% 
  filter(!is.na(del11q)) %>% 
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = del11q, values_from = n)
```
...
patMeta_cl %>% 
  ...
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = trisomy12, values_from = n) 
```
...
patMeta_cl %>% 
  ...
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = POT1, values_from = n) 
```
...
patMeta_cl %>% 
  ...
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = SF3B1, values_from = n) 
```
...
patMeta_cl %>% 
  ...
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = gain8q, values_from = n) 
```
...
patMeta_cl %>% 
  ...
  count() %>% 
  ungroup() %>% 
  pivot_wider(names_from = TP53, values_from = n) 
```
...
patMeta_cl %>% 
  select(PatientID, Cluster, IGHV.status, trisomy12, POT1, SF3B1, gain8q, RAS_RAF, TP53, ATM) %>% 
  
  ggplot()+
    ggmosaic::geom_mosaic(aes(x = product(Cluster), fill=IGHV.status))
```
