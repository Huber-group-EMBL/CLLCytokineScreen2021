 ...
 ann_colors = 
   list(
    "IGHV status" = c("Unmutated"=IGHV[1],
                      "Mutated"=IGHV[2]),
    "DNA Methylation"= c("Low"=Methylation_cluster[1],
...
Plot Figure
```{r Fig2A, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=16, fig.height=12, warning = FALSE}
...
Additional reduced version of Fig2A for synopsis image
```{r, fig.width=3, fig.height=1.5, eval=FALSE}
Heatmap_Annotation_short<-select(Heatmap_Annotation, Cluster)
pheatmap(viab.mat.lims,  
         scale = "none",
         clustering_method = "complete", 
         cluster_cols = TRUE,
         ...
         cutree_cols = 4,
         clustering_distance_cols = cluster_matrix,
         annotation_col = Heatmap_Annotation_short,
         annotation_colors = ann_colors,
...
Fig2B <-
  ...
  scale_y_log10() +
  guides(fill="none") +
  ylab("Lymphocyte doubling time in years") +
  t2 +
  stat_compare_means(method="t.test", comparison=list(c(1,2),c(3,4)), size=6, label.x = 1.3, label.y=2.1, method.args=list(paired=FALSE, alternative="two.sided"))
Fig2B
```

#### Print median LDT of clusters for text reference
```{r}
  patMeta_cl %>%
  #join patient meta data with clusters, to LDT dataframe
  left_join(  dplyr::select(LDT, PatientID, doubling.time), by=c("PatientID"="PatientID")) %>%
  ...
  dplyr::mutate(Cluster=as.factor(Cluster)) %>%
  group_by(Cluster) %>% 
  summarize(median.doubling.time=round(median(doubling.time), 2), numberofsamples=n())
...
t_plot<-  t2+theme(plot.title=element_blank(), axis.title.x=element_blank(), legend.key = element_blank(), legend.text= element_text(size=14))
...
```{r, eval=FALSE}
patMeta_cl %>% 
  select(Cluster, IGHV.status) %>% 
  group_by( Cluster, IGHV.status) %>% 
  filter(!is.na(IGHV.status)) %>% 
  count() %>% 
  ungroup()
patMeta_cl %>% 
  select(Cluster, trisomy12) %>% 
  group_by( Cluster, trisomy12) %>% 
  filter(!is.na(trisomy12)) %>% 
  count() %>% 
  ungroup() %>% 
...
```{r }
# load("../../data/multinominal_regression_Clusters_Fig2D.RData" )
>>>>>>> 1e64f538c637f62c254f8d7afae53e9f70f566d7
patMeta_cl %>% 
  select(Cluster, TP53) %>% 
  group_by( Cluster, TP53) %>% 
  filter(!is.na(TP53)) %>% 
...
<<<<<<< HEAD
  filter(Cluster==4) 
patMeta_cl %>% 
  select(Cluster, RAS_RAF) %>% 
  group_by( Cluster, RAS_RAF) %>% 
  filter(!is.na(RAS_RAF)) %>% 
  count() %>% 
  ungroup() %>% 
  filter(Cluster==4) 
patMeta_cl %>% 
  select(Cluster, gain8q) %>% 
  group_by( Cluster, gain8q) %>% 
  filter(!is.na(gain8q)) %>% 
  count() %>% 
  ungroup() %>% 
  filter(Cluster==4)
...
```{r}
plotTab$label<-NA
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="1"),which(colnames(plotTab)=="label")]<-"20/71"
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="2"),which(colnames(plotTab)=="label")]<-"6/25"
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="3"),which(colnames(plotTab)=="label")]<-"28/37"
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="4"),which(colnames(plotTab)=="label")]<-"45/49"
plotTab[which(plotTab$feature=="trisomy 12"&plotTab$cluster=="2"),which(colnames(plotTab)=="label")]<-"9/21"
plotTab[which(plotTab$feature=="SF3B1"&plotTab$cluster=="2"),which(colnames(plotTab)=="label")]<-"6/25"
plotTab[which(plotTab$feature=="POT1"&plotTab$cluster=="3"),which(colnames(plotTab)=="label")]<-"3/37"
plotTab[which(plotTab$feature=="ATM"&plotTab$cluster=="4"),which(colnames(plotTab)=="label")]<-"1/46"
plotTab[which(plotTab$feature=="TP53"&plotTab$cluster=="4"),which(colnames(plotTab)=="label")]<-"2/49"
plotTab[which(plotTab$feature=="RAS/RAF"&plotTab$cluster=="4"),which(colnames(plotTab)=="label")]<-"0/49"
plotTab[which(plotTab$feature=="gain(8q)"&plotTab$cluster=="4"),which(colnames(plotTab)=="label")]<-"0/47"
    
plotTab$yposition<-NA
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="1"),which(colnames(plotTab)=="yposition")]<-.5
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="2"),which(colnames(plotTab)=="yposition")]<-.5
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="3"),which(colnames(plotTab)=="yposition")]<--.5
plotTab[which(plotTab$feature=="IGHV mutated"&plotTab$cluster=="4"),which(colnames(plotTab)=="yposition")]<--.5
plotTab[which(plotTab$feature=="trisomy 12"&plotTab$cluster=="2"),which(colnames(plotTab)=="yposition")]<--.5
plotTab[which(plotTab$feature=="SF3B1"&plotTab$cluster=="2"),which(colnames(plotTab)=="yposition")]<--.5
plotTab[which(plotTab$feature=="POT1"&plotTab$cluster=="3"),which(colnames(plotTab)=="yposition")]<--.5
plotTab[which(plotTab$feature=="ATM"&plotTab$cluster=="4"),which(colnames(plotTab)=="yposition")]<-0.5
plotTab[which(plotTab$feature=="TP53"&plotTab$cluster=="4"),which(colnames(plotTab)=="yposition")]<-0.5
plotTab[which(plotTab$feature=="RAS/RAF"&plotTab$cluster=="4"),which(colnames(plotTab)=="yposition")]<-0.5
plotTab[which(plotTab$feature=="gain(8q)"&plotTab$cluster=="4"),which(colnames(plotTab)=="yposition")]<-0.5
```

```{r Fig2D,  fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height=6, fig.width=7}
Fig2D<-   ggplot(plotTab, aes(x=feature, y=meanCoef))+ 
   ...
   theme(strip.text = element_text(size = fontsize),
    ...
    facet_wrap(~cluster, 
      ...
      geom_text(aes(y=yposition, label=label), color="black", size=5 , fontface="bold")+
   guides(fill = "none")
