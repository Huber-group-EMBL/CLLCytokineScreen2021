...
```{r,  fig.path=plotDir, dev=c("png", "cairo_pdf"),  fig.width=9, fig.height=9}
...
coefficients.all %>% 
  filter(Drug=="Ibrutinib", Cytokine=="Interferon gamma") %>% 
...
df %>% 
  filter(treatment_cytokine%in%c("Interferon gamma", "IL-4")) %>% 
  select(PatientID, treatment_drug, treatment_cytokine, Viability) %>% 
  pivot_wider(names_from = "treatment_cytokine", values_from = "Viability") %>% 
  
  ggplot(aes(x=`IL-4`, y=`Interferon gamma`))+
  geom_point()+
  geom_smooth(method="lm", formula='y ~ x')+
  stat_cor()+
  facet_wrap(.~treatment_drug, scales = "free")
```


```{r, fig.height=12, fig.width=12}
df %>% 
  filter(treatment_cytokine%in%c("No Cytokine", "IL-4")) %>% 
  select(PatientID, treatment_drug, treatment_cytokine, Viability) %>% 
  pivot_wider(names_from = "treatment_cytokine", values_from = "Viability") %>% 
  
  ggplot(aes(x=`IL-4`, y=`No Cytokine`))+
  geom_point()+
  geom_smooth(method="lm", formula='y ~ x')+
  stat_cor()+
  facet_wrap(.~treatment_drug, scales = "free")
...
```{r, fig.height=12, fig.width=12}
coefficients.all %>% 
  filter( Cytokine%in%c("IL-2","IL-15")) %>% 
  # left_join(patMeta, by = "PatientID") %>% 
  pivot_wider(names_from = "Cytokine", values_from = "beta") %>% 
  
  
  ggplot(aes(x=`IL-2`, y=`IL-15`))+
  geom_point()+
  geom_smooth(method="lm", formula='y ~ x')+
  facet_wrap(.~Drug, scales = "free")+
  stat_cor()
