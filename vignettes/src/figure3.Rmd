...
<!-- ```{r} -->
<!-- data_tmp<-dplyr::select(df, PatientID, Log, Cytokine) %>% -->
...
<!--   filter(Cytokine=="TGF-b1", !is.na(SPEN)) -->

<!-- var.test(Log ~ SPEN, data_tmp, alternative = "two.sided") -->

<!--   ggplot(data_tmp, aes(x=SPEN, y=Log))+ -->
<!--   geom_boxplot()+ -->
<!--     geom_beeswarm()+ -->

<!--   stat_compare_means(comparisons = list(c(1,2)), method = "t.test", method.args = list(var.equal = T))+ -->
<!--   t2+ -->
<!--     ggtitle("TGF-b1 effect by SPEN mutation status.\nT-test with equal variance.") -->
<!-- ``` -->

<!-- ```{r} -->
<!--   ## Select columns from screening data  -->
<!--   var_test<-dplyr::select(df, PatientID, Log, Cytokine) %>%  -->
...
<!--   ## Perform t.test on every combination of Cytokine and genetic alteration -->
<!--   do(tidy(var.test(Log ~ alt_value, data = .,alternative = "two.sided"))) %>%  -->
...
<!--   ggplot(var_test, aes(x=p.value))+ -->
<!--   geom_histogram(bins=100, aes(fill=ifelse(adj.p.value<0.05, "p-value<0.05", "p-value>0.05")))+ -->
<!--   guides(fill="none")+ -->
<!--   t2+ -->
<!--     ggtitle("Variance testing for all associations between \ngenetic alterations and stimuli. \nHistogram of non-adjusted p-values.\nAdj.p.values <0.05 colored red.")  -->

<!--   var_test %>%  -->
<!--   dplyr::filter(adj.p.value<0.1) %>%  -->
<!--   dplyr::select(Cytokine, Genetic_alt,adj.p.value, p.value) %>%  -->
<!--   dplyr::arrange(adj.p.value) %>%  -->
<!--   DT::datatable() -->
