...
### Fig3A including Complex Karyotype

```{r, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height = 10, fig.width = 10, message = FALSE, warning = FALSE}
##################### List of mutations with >2 postive cases #############################
selected_mutations <- 
  patMeta %>% 
  mutate(Ras_Raf=as.factor(ifelse(BRAF==1|KRAS==1|NRAS==1, 1,0))) %>% 
...
```{r, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height = 10, fig.width = 10, message = FALSE, warning = FALSE}
p_values %>% 
  filter(Genetic_alt=="Complex_Karyotype3") %>%
  select(Cytokine, Genetic_alt, p.value, adj.p.value) %>% 
  arrange(adj.p.value)%>% 
  DT::datatable()
```
