...
```{r S1, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=10, fig.height=5, fig.pos="H"}
...
```{r S2,fig.path=plotDir, dev=c("png", "cairo_pdf"),  fig.pos="H", fig.height=8, fig.width=16}
...
```{r S3, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.pos="H", fig.width=11, fig.height=11}
...
```{r, S5, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height=7, fig.width=7}
...
## Appendix Figure 5. Genetic features of microenvironmental response clusters. 
Multinomial regression with lasso penalisation to identify enrichment or depletion of genetic features within each cluster.
...
## Appendix Figure 6. Methylation profile of Clusters defined by response to microenvironmental stimuli
...
```{r S7, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height=13, fig.width=10.5}
...
## Appendix Figure 7. CLL Proliferative Drive as defined by Lu et al., 2021^5^.
...
```{r S8, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=14.5, fig.height=11, warning = FALSE}
...
## Appendix Figure 8. Drug response between clusters.
...
```{r S9, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=11, fig.height=12}
...
## Appendix Figure 9. Drug response by clusters.
...
```{r S10, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height= 14, fig.width = 12, warning=F, message=F, cache=FALSE}
...
## Appendix Figure 10. RNA-Sequencing of matched samples indicates differential gene expression between  Cluster 3 and Cluster 4.
...
```{r S11, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height= 7, fig.width = 12, warning=F, message=F, cache=FALSE}
...
## Appendix Figure 11. Comparison of gene expression between Cluster 1 and 2.
...
```{r S12,fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=12, fig.height=4, message=FALSE, warning=FALSE}
...
## Appendix Figure 12. Predictor profiles to represent gene - stimulus associations. 
...
```{r S13, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=10, fig.height=7, warning=F}
...
## Appendix Figure 13. Correlation of stimuli response and receptor expression.
...
```{r S14,fig.path=plotDir, dev=c("png", "cairo_pdf"),  fig.height=7, fig.width=12}
...
## Appendix Figure 14. BCR inhibition by ibrutinib counters the protective effect of TLR stimulation in all genetic subgroups of IGHV and trisomy 12.
...
```{r S15, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height = 6, fig.width = 10}
...
```{r S16, fig.path=plotDir, dev=c("png", "cairo_pdf") fig.height = 6, fig.width = 10, eval = FALSE}
...
## Appendix Figure 16. ATACseq comparing trisomy 12 and non-trisomy 12 CLL samples.
Volcano plot shows change in inferred TF activity (x axis) against adjusted p-values (y axis) for  trisomy 12 (n = 2) versus non-trisomy 12 samples (n = 2). Inferred TF activity calculated using the diffTF package, and measured as weighted mean difference. p-values are obtained through diffTF in analytic mode and adjusted by the Benjamini-Hochberg procedure. TFs are labeled where absolute weighted mean difference > 0.15, and adjusted p-value < 0.01. Data from Beekman et al 2018. 

\newpage


```{r S17, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height = 6, fig.width = 10}
...
## Appendix Figure 17. ATACseq comparing trisomy 12 and non-trisomy 12 CLL samples, without including TF binding sites on chromosome 12.
Volcano plot shows change in inferred TF activity (x axis) against adjusted p-values (y axis) for  trisomy 12 (n = 2) versus non-trisomy 12 samples (n = 2). Inferred TF activity calculated using the diffTF package, and measured as weighted mean difference.  p-values are obtained through diffTF in analytic mode and adjusted by the Benjamini-Hochberg procedure. TFs are labelled where absolute weighted mean difference > 0.15, and adjusted p-value < 0.01. (A) For the two versus two comparison (B) For the Nine versus 42 comparison based on the data from Rendeiro et al. 2016. 


\newpage

```{R, S11, fig.width=15, fig.height=10, message=FALSE }
...
## Appendix Figure 17.  Over-representation tests of all KEGG and Reactome pathways in ChIPseq analysis of Spi-B binding in lymphoma cell lines.
Results shown for pathways containing the most Spi-B gene targets (top 20).
