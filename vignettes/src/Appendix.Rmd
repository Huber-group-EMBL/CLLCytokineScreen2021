...
lassoPlot <- function(lassoOut, geneMatrix, betaMatrix, freqCut = 1, coefCut = 0.01) {
  ...
  xlab(expression(paste("Patient-specific $\beta_{int}$ ")))
...
```{r lassoPlot_CLLPD}
 ...
 lassoPlot_CLLPD<- function(lassoOut, viabMatrix, PDMatrix, freqCut = 1, coefCut = 0.01) {
    ...
    matValue$Sample <- factor(matValue$Sample, levels = names(PDValue))
     ...
     #plot the heatmap 
      ...
      p1 <- ggplot(matValue, aes(x=Sample, y=Var)) + 
            ...
            ylab("") +
            scale_fill_gradient2(low = palblues[1],high = palreds[8], mid = "white", name="Z-score")
    ...
    # construct the gtable
    wdths = c(0.2, 1.5, 0.2, 1.3*ncol(matValue), 1.5, 1)
    hghts = c(0.2, 0.2, 0.0020*nrow(matValue), 0.3, 1, 0.2)
    gt = gtable(widths=unit(wdths, "in"), heights=unit(hghts, "in"))
    ...
    gt = gtable_add_grob(gt, gtable_filter(gg1, "axis-l"), 3, 5) # variable names
    gt = gtable_add_grob(gt, gtable_filter(gg1, "xlab-b"), 2, 4) # axis title
    gt = gtable_add_grob(gt, gtable_filter(gg1, "guide-box"), 3, 6) # scale
...
#diffTF_Beekman: tibble containing results from running diffTF package on Beekman data
load( "../../data/diffTF_Beekman.RData")
...
#BIGCLL52.SPI1.output: tibble containing Lof2FCs and p-values for each PU.1 (SPI-1) binding site
load("../../data/BIGCLL52.SPI1.output.RData")
...
## remove unused columns from metadata
  ...
  pivot_wider(names_from = alt_value, values_from = n) %>% 
   ...
   rename('Genetic feature'=Genetic_alt, 'Wildtype/IGHV unmutated'="0", 'Mutated/ IGHV Mutated'="1", 'Not available'='NA')%>% 
...
```
## Appendix Table S1. List of genetic features tested for their association with stimulus response in the univariate model of Figure 3A.
Only genetic features with at least three positive cases were considered. 
...
colnames(cox_output) <- c("Factor", "Hazard Ratio", "p-value", "CI Low", "CI High")
...
## Appendix Table S2. Survival analysis of microenvironmental response clusters comparing C3 to C1, C2 and C4.
Cox proportional hazards model of stimulus response clusters using time to next treatment and with C3 as reference.
...
cox_output %>%  
  mutate(Factor=gsub(pattern=", \"3\"", "",Factor)) %>% 
  kable() %>%
  kable_styling(latex_options = c("striped", "HOLD_position")) 

```

## Appendix Table S3. Survival analysis of microenvironmental response clusters comparing C3 to C1 and C2 combined.
Cox proportional hazards model of stimuli response clusters using TTT.
...
c <- 
  clusters.surv %>%
  mutate(Cluster=factor(Cluster)) %>%
  coxph(Surv( TTT, treatedAfter) ~  factor2ind(Cluster, 3)+ IGHV.status + trisomy12 + TP53+Methylation_Cluster+POT1,  data=.)
...
## Appendix Table S4. Multivariate survival analysis of response clusters.
Multivariate Cox proportional hazards model of stimulus response clusters and genetic subgroups of disease progression using time to next treatment  with C3 as reference.
...
TFBS.PU1 <- BIGCLL52.SPI1.output %>% dplyr::filter(abs(l2FC) >1, pval_adj <0.1)
...
peakAnno <- annotatePeak(peak, 
                         tssRegion=c(-3000, 3000), #region in which to look for genes
...
peakAnno_PU1 <- annotatePeak(peak_pu1, 
                         tssRegion=c(-3000, 3000), #region in which to look for genes
                         TxDb=txdb, #define where to get transcript info
                         annoDb="org.Hs.eg.db", addFlankGeneInfo = TRUE, verbose = FALSE) #set annotation package to add ensembl, smbol and genames
...
pu1Res <- enricher(unique(as.data.frame(peakAnno_PU1)$geneId), 
                    #Use pathways in term2gene
                    TERM2GENE=term2gene, 
                    #assign description for each term, as its database source
                    TERM2NAME = term2genesetdatabase, 
...
resTab.pu1 <-  
  mutate(pu1Res.df, genesetSize = BgRatio*length(unique(term2gene$gene))) %>%  #multiply BgRatio by total genes tested to get number of genes in set
  dplyr::select(ID, Description, pvalue, p.adjust, Count, genesetSize)

colnames(resTab.pu1) <- c("ID", "genesetDatabase", "pvalue_PU1", "adjpvalue_PU1","count_PU1", "genesetSize")


#where count and p-value is NA, put a 0 / 1 (ie there were no SPIB target genes in the set)
resTab.spib$count_SPIB[is.na(resTab.spib$count_SPIB)] <- 0
resTab.spib$pvalue_SPIB[is.na(resTab.spib$pvalue_SPIB)] <- 1
resTab.spib$adjpvalue_SPIB[is.na(resTab.spib$adjpvalue_SPIB)] <- 1

#where count and p-value is NA, put a 0 / 1 (ie there were no PU1 target genes in the set)
resTab.pu1$count_PU1[is.na(resTab.pu1$count_PU1)] <- 0
resTab.pu1$pvalue_PU1[is.na(resTab.pu1$pvalue_PU1)] <- 1
resTab.pu1$adjpvalue_PU1[is.na(resTab.pu1$adjpvalue_PU1)] <- 1


#select columns of interest to show in figure
resTab.spib <- dplyr::select(resTab.spib, ID, genesetDatabase, genesetSize,  count_SPIB, pvalue_SPIB, adjpvalue_SPIB)

#select columns of interest to show in figure
resTab.pu1 <- dplyr::select(resTab.pu1, ID, genesetDatabase, genesetSize,  count_PU1, pvalue_PU1, adjpvalue_PU1)

#adjust decimal places of p-values
resTab.spib$adjpvalue_SPIB <- round(resTab.spib$adjpvalue_SPIB, digits = 3)
resTab.spib$pvalue_SPIB <- round(resTab.spib$pvalue_SPIB, digits = 3)

#adjust decimal places of p-values
resTab.pu1$adjpvalue_PU1 <- round(resTab.pu1$adjpvalue_PU1, digits = 3)
resTab.pu1$pvalue_PU1 <- round(resTab.pu1$pvalue_PU1, digits = 3)


allresTab <- left_join(resTab.spib, resTab.pu1)

#where count and p-value is NA, put a 0 / 1 (ie there were no PU1 target genes in the set)
allresTab$count_PU1[is.na(allresTab$count_PU1)] <- 0
allresTab$pvalue_PU1[is.na(allresTab$pvalue_PU1)] <- 1
allresTab$adjpvalue_PU1[is.na(allresTab$adjpvalue_PU1)] <- 1

allresTab <- select(allresTab, "ID", "genesetDatabase", "genesetSize", "count_SPIB", "pvalue_SPIB", "adjpvalue_SPIB","count_SPIB" , "count_PU1", "pvalue_PU1", "adjpvalue_PU1" )


allresTab$pvalue_SPIB <- ifelse(allresTab$pvalue_SPIB >0.01, allresTab$pvalue_SPIB, paste0("**", as.character(allresTab$pvalue_SPIB), "**")) 
allresTab$adjpvalue_SPIB <- ifelse(allresTab$adjpvalue_SPIB >0.01, allresTab$adjpvalue_SPIB, paste0("**", as.character(allresTab$adjpvalue_SPIB), "**")) 

allresTab$pvalue_PU1 <- ifelse(allresTab$pvalue_PU1 >0.01, allresTab$pvalue_PU1, paste0("**", as.character(allresTab$pvalue_PU1), "**")) 
allresTab$adjpvalue_PU1 <- ifelse(allresTab$adjpvalue_PU1 >0.01, allresTab$adjpvalue_PU1, paste0("**", as.character(allresTab$adjpvalue_PU1), "**")) 


colnames(allresTab) <- c("Pathway",
                      "Geneset\nDatabase" , 
                      "Geneset\nSize",  
                      ...
                      "Spi-B p-value",
                      "Spi-B adj. p-value",
                      #show the total number of PU.1 targets in ChIPseq data 
                      paste("PU.1 targets\n(/" ,length(unique(as.data.frame(peakAnno_PU1)$geneId)),")", sep= ""),
                      "PU.1 p-value",
                      "PU.1 adj. p-value")
...
#if p value <-, reformat as **value** 

``` 

## Appendix Table S5. Over-representation tests of immune signalling pathways amongst Spi-B and PU.1 gene targets, based on primary CLL ATACseq data.  
Gene sets were selected from KEGG and Reactome databases. Control pathways i.e not related to immune signalling (Oxidative Phosphorylation, TP53 signalling and DNA Replication) were also included in the analysis and are not significant. 25 genes sets were tested, results for all genesets containing at least one target gene are shown.
...
allresTab <- left_join(resTab, resTab.pu1)
...
#adjust decimal places of p-values
allresTab$adjpvalue_SPIB <- round(allresTab$adjpvalue_SPIB, digits = 3)
allresTab$pvalue_SPIB <- round(allresTab$pvalue_SPIB, digits = 3)
allresTab$adjpvalue_PU1 <- round(allresTab$adjpvalue_PU1, digits = 3)
allresTab$pvalue_PU1 <- round(allresTab$pvalue_PU1, digits = 3)

#get significant results
allresTab <- select(allresTab, ID, genesetDatabase, genesetSize,  count_SPIB, pvalue_SPIB, adjpvalue_SPIB, count_PU1, pvalue_PU1, adjpvalue_PU1)
...
colnames(allresTab) <- c("Pathway",
                      ...
                      "SPIB\n p-value",
                      "SPIB\n adj. p-value", 
                       paste("PU.1 targets\n(/" ,length(unique(as.data.frame(peakAnno.pu1.1kb)$geneId)),")", sep= ""),
                      "PU.1\n p-value",
                      "PU.1\n adj. p-value")
...
## Appendix Table S6. Over-representation tests of immune signalling pathways amongst Spi-B and PU.1 gene targets, based on DLBCL ChIPseq data from Care et al. 2014^1^. 
Gene sets were selected from KEGG and Reactome databases. Control pathways i.e not related to immune signalling (Oxidative Phosphorylation, TP53 signalling and DNA Replication) were also included in the analysis and are not significant. 25 genesets were tested, all are shown.
...
## Appendix Figure S1. Genetic profiles of screened patient samples.
Selected genetic features on y-axis and patient samples (n=192) on x-axis.
\newpage
...
p1<-df_patMeta%>%
  ...
  mutate(IGHV.status=factor(IGHV.status, levels = c("U","M"), labels = c("unmutated", "mutated"))) %>% 
  ...
  geom_violin(cex=0.4, alpha=0.7,  aes(x=Cytlabel,y=Log, color = IGHV.status, fill = IGHV.status))+
   geom_text(aes(label = Sig, y = 2), size = 6, data = filter(t_tests, Cytlabel%in%c("Resiquimod","IL4", "sCD40L + IL4","CpG ODN")))+
      scale_colour_manual(name = 'IGHV status', values = setNames(c(palblues[1],palreds[8]), c("mutated", "unmutated"))) +
      scale_fill_manual(name = 'IGHV status', values = setNames(c(palblues[1],palreds[8]), c("mutated", "unmutated"))) +
   theme_minimal()+
...
p2<-df_patMeta%>%
  ...
  geom_violin(cex=0.4, alpha=0.7,aes(x=Cytlabel, y=Log, colour = IGHV.status, fill = IGHV.status))+
   geom_text(aes(label = Sig, y = 0.625), size = 6, data = filter(t_tests, Cytlabel%in%c("IL1\u03B2","Interferon \u03B3","Interferonγ","SDF-1\u03B1","sCD40L","soluble anti-IgM","IL6","IL10","IL21","HS-5 CM","IL15","BAFF","IL2", "TGF\u03B21")))+
...
## Appendix Figure S2. Response to stimuli stratified by IGHV status.
Logarithm of the relative viability after 48h incubation with microenvironmental stimuli, stratified by IGHV status (IGHV mutated: n=99; IGHV unmutated: n=83). BH-transformed p-values are shown from one-sample Student's t-tests of all patient samples. (p<0.00001=&#42;&#42;&#42;&#42;&#42;, p<0.0001=&#42;&#42;&#42;&#42;, p>0.05=ns) 



```{r S3, fig.path=plotDir, dev=c( "cairo_pdf"), fig.pos="H", fig.width=11, fig.height=12}

example_cyt<-c("Resiquimod","CpG ODN","IL1\u03B2", "Interferon\u03B3","sCD40L","TGF\u03B21","IL4", "soluble anti-IgM", "IL15","sCD40L + IL4","IL21", "HS-5 CM","BAFF", "IL6", "SDF-1\u03B1","IL10","IL2")
...
## Appendix Figure S3. Stimulus response stratified by clusters.
Logarithm of the relative viability after treatment with stimuli, faceted by cluster. BH-transformed p-values from Student's t-test (C1: n=77; C2:n=27; C3: n=38; C4: n=50).
...
## Appendix Figure S4. Time to first treatment and overall survival by clusters.
...
#get only viability values for patients that have a PD
viabMat <- viabMat[colnames(PDmat),]


CLLPD_Lasso_all_patients <-
  lassoPlot_CLLPD(dataResult, 
...
```{r S5, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height=13, fig.width=13}
...
## Appendix Figure S5. The relationship between CLL Proliferative Drive (as defined by Lu et al. (2021)^2^) and microenvironmental response.
**(A)** Beeswarm-boxplot of CLL-PD values stratified by cluster. P-values are from Student's t-tests (C1: n=57; C2:n=20; C3: n=29; C4: n=40). **(B-D)** Predictor profiles depicting relationship between CLL-PD and stimulus response. Stimuli that were associated to the size of CLL-PD were identified with Gaussian linear modelling with L1-penalty, using viability z-scores for each stimulus as feature matrix, and CLL-PD values as response matrix (n=146). Predictor profiles show  z-scores of logarithm of relative viability for each stimuli associated with CLL-PD, bar plot on left shows size of coefficient assigned to each stimulus and scatter plot below indicates CLL-PD values for same patient sample. Coefficients are mean of 30 bootstrapped model fits, plotted when selected in >75% of fits.  **(C)** shows model outputs for  IGHV-M only samples   (n=80) and **(D)**  for IGHV-U only  samples (n=65). **(E & F)**  Exemplary scatter plots split by IGHV mutation status shown for Resiquimod **(E)** and IL1ß **(F)** (IGHV-M: n=80, IGHV-U: n=65). CLL-PD on x-axis and the logarithm of relative viability after microenvironmental stimulation on y-axis. Pearson correlation coefficients are shown with p-values. 
...
```{r S6, fig.path=plotDir, dev=c("png", "cairo_pdf"),fig.height=5}
plotTab<-patMeta_Cluster %>% 
  ...
  mutate(Methylation_Cluster=factor(Methylation_Cluster, levels = c("HP", "IP", "LP"), labels = c("High programmed", "Intermediate programmed", "Low programmed")))


  ggplot(plotTab, aes(x=Cluster, y=freq, fill=Methylation_Cluster))+
  geom_bar(stat="identity")+
  ylab("Relative incidence of\n DNA methylation profiles")+
  scale_fill_manual(name = 'DNA Methylation Profile', values = Methylation_cluster)+
  t2
```

## Appendix Figure S6. DNA Methylation profile of Clusters defined by response to microenvironmental stimuli
Barplot of DNA methylation profiles in the four clusters of microenvironmental response. C1 and C2 show higher abundance of low programmed (LP) DNA Methylation profiles, while C3 and C4 show more highly programmed (HP) DNA methylation clusters (C1: n=77; C2:n=27; C3: n=38; C4: n=50). NA=Not available. 

\newpage


```{r S7, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height= 14, fig.width = 12, warning=F, message=F, cache=FALSE}
...
ggplot( aes(x=log2FoldChange, y=-log10(padj))) +
      ...
      t2 +
      labs(title="Cluster 3 versus 4", x = "log2 fold change", color = "Significance")+
      guides(color="none", alpha="none")+
  scale_y_continuous("BH-transformed p-value", 
                     breaks=seq(0,10,5),
...
\newpage
## Appendix Figure S7. RNA-Sequencing of matched samples indicates differential gene expression between  Cluster 3 and Cluster 4.
Volcano plot of differentially expressed genes between Cluster 3 and Cluster 4 (C3: n=9; C4: n=12) **(A)**. X axis indicates log2 fold change values, calculated using the DESeq2 package, y axis gives corresponding -log10(transformed p-value). P-values transformed using BH method. Genes are labeled where p-value < 0.05. Enrichment plots of selected pathways **(B-D)**. Gene set enrichment analysis (GSEA) was performed with the Hallmark gene sets from the GSEA Molecular Signatures Database. Wald statistic was used to rank the genes.  The green curve corresponds to the Enrichment Score curve, which is the running sum of the weighted enrichment score obtained from GSEA software. 
\newpage

```{r S8, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height= 7, fig.width = 12, warning=F, message=F, cache=FALSE}
...
ggplot( aes(x=log2FoldChange, y=-log10(padj))) +
      ...
      scale_colour_manual(values=c(palblues[2], palreds[4], "black", "grey"))+
      t2 +
      labs(title="Cluster 1 versus 2", x = "log2 fold change", color = "Significance")+
      guides(color="none", alpha="none")+
  scale_y_continuous("BH-transformed p-value", 
                     breaks=seq(0,10,5),
                     labels=math_format(expr=10^.x)(-seq(0,10,5)))
...
## Appendix Figure S8. RNA-Sequencing of matched samples indicates differential gene expression between Cluster 1 and 2.
Volcano plot of differentially expressed genes between Cluster 1 and Cluster 2 (C1: n=17; C2: n=11). X axis indicates log2 fold change values, calculated using the DESeq package, y axis gives corresponding -log10(transformed p-value). P-values transformed using BH method. Genes are labeled where p-value<0.05.
...
```{r S9, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=14.5, fig.height=11, warning = FALSE}
...
## Appendix Figure S9. Drug response between clusters.
Heatmap showing the logarithm of relative viability after drug treatment. Rows represent drug treatments and columns represent primary CLL samples (n=192), annotated for their genetic background, sex and pretreatment status above. Red values indicate increased viability upon treatment, blue indicates decreased viability.

```{r S10, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=11, fig.height=12}
...
## Appendix Figure S10. Drug response by clusters.
Logarithm of the relative viability after treatment with drugs, faceted by cluster. BH-transformed p-values from non-paired, two-sided Student's t-test are shown (C1: n=77; C2:n=27; C3: n=38; C4: n=50).




```{r S11, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height=6, fig.width=8}
...
## Appendix Figure S11. Effect of BAY-11-7085 treatment on BAFF response
Logarithm of the relative viability after treatment BAFF, BAY-11-7085 or both, faceted by cluster. P-values from paired, two-sided Student's t-test are shown (C1: n=77; C2:n=27; C3: n=38; C4: n=50).
...
CR_volcanoplot <-
  ...
  scale_y_continuous("BH-transformed p-value", 
                     breaks=seq(0,10,1),
                     labels=math_format(expr=10^.x)(-seq(0,10,1)))
...
## Appendix Figure S12. Correlation of stimulus response and receptor expression.
Volcano plot depicts Pearson correlation coefficients against corresponding BH-transformed p-values, for the correlation between the logarithm of the relative viability and vst RNA counts of corresponding stimulus receptor (n=49). RNA counts taken from RNA-Sequencing of untreated matched CLL patient sample. Only viability  after treatment with Resiquimod correlated with receptor expression (R>0.3).

\newpage

```{r S13,fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.width=12, fig.height=4, message=FALSE, warning=FALSE}
...
atac.plot =
  ...
  t2 +
  ggtitle("Trisomy 12 v Non-Trisomy 12") +
  xlab("Change in inferred TF activity") +
  guides(color="none", alpha="none") +
  coord_cartesian(ylim = c(0,6.5))+
  scale_y_continuous("BH-transformed p-value", 
                     breaks=seq(0,10,3),
                     labels=math_format(expr=10^.x)(-seq(0,10,3)))
...
## Appendix Figure S14. ATACseq comparing trisomy 12 and non-trisomy 12 CLL samples.
Volcano plot shows change in inferred TF activity (x-axis) against transformed p-values (y-axis) for  trisomy 12 (n = 2) versus non-trisomy 12 samples (n = 2). Inferred TF activity calculated using the diffTF package,  TFs are labeled where inferred activity > 0.15, and BH-transformed p-value < 0.01.


\newpage

```{r S15, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height = 6, fig.width = 10, eval=TRUE}
...
atac.plot =
  ...
  geom_point(aes(alpha=0.4, colour = ifelse(-log10(pvalueAdj) > 1.3,"black","grey")),size=3) +
  geom_label_repel(aes(label=ifelse(-log10(pvalueAdj) > 1.3&abs(weighted_meanDifference)>0.1,
                                    as.character(TF),''),
                       colour=ifelse(weighted_meanDifference>0,
                                     palreds[1],
                                     palblues[1])),
...
#check number of upregulated TFs
#nrow(filter(diffTF_Beekman, weighted_meanDifference>0, pvalueAdj <0.05))
```

## Appendix Figure S15. ATACseq comparing trisomy 12 and non-trisomy 12 CLL samples.
Volcano plot shows change in inferred TF activity (x-axis) against transformed p-values (y-axis) for  trisomy 12 (n = 13) versus non-trisomy 12 samples (n = 87). Inferred TF activity calculated using the diffTF package, and measured as weighted mean difference. p-values are obtained through diffTF in permutation mode and transformed by the Benjamini-Hochberg procedure. TFs are labeled where absolute weighted mean difference > 0.1, and transformed p-value < 0.05. Data from Beekman et al. 2018^4^. 
...
## Appendix Figure S16. ATACseq comparing trisomy 12 and non-trisomy 12 CLL samples, independent of third copy of chromosome 12
Volcano plot shows change in inferred TF activity (x axis) against transformed p-values (y axis) for  trisomy 12 versus non-trisomy 12 samples, for (A) the 2 versus 2 comparison and (B) the 9 versus 42 comparison based on the data from Rendeiro et al. 2016^3^.  Input peak files supplied to diffTF did not include peaks on chromosome 12. p-values are obtained through diffTF in (A) analytic mode and (B) permutation mode, transformed by the Benjamini-Hochberg procedure. TFs are labelled in (A) where absolute weighted mean difference > 0.15, and transformed p-value < 0.01 and in (B) where absolute weighted mean difference > 0.05, and transformed p-value < 0.1.


\newpage

```{r Spibandpu1_binding_sites}
...
#filter PU1 binding sites  for those that are sig between two conditions, and show a sig Log2FC
TFBS.PU1 <- BIGCLL52.SPI1.output %>% dplyr::filter(abs(l2FC) >1, pval_adj <0.1)
...
```{r S17, fig.path=plotDir, dev=c("cairo_pdf"), fig.height = 15, fig.width = 15, warning = FALSE}
...
peak_df%>% 
 ...
 ggplot(aes(start/10^6, l2FC))+
  ...
  ggtitle("Change in inferred TF activity \nfor Spi-B motifs by Chromosome")+
  t2 + 
...
#make factors / numeric
chr.sorted <- unique(sortChrName(peak_df_PU1$chr))
peak_df_PU1$chr <- factor(peak_df_PU1$chr, levels = chr.sorted)
peak_df_PU1$start <- as.numeric(peak_df_PU1$start)
peak_df_PU1$end <- as.numeric(peak_df_PU1$end)


chr_length<-filter(human.hg19.genome, V1%in%levels(peak_df_PU1$chr)) %>% 
         mutate(V1=factor(V1, levels = rev(levels(peak_df_PU1$chr)))) %>% 
          mutate(chromosome=gsub("chr","Chromosome ",V1)) %>% 
  rename(chr_length=V2) %>% 
  select(-V1)
...
## Appendix Figure S17. Chromosomal locations of Spi-B and PU.1 motifs that show differential accessibility between trisomy 12 and non-trisomy 12 CLL. 
Log2 fold changes are plotted (y axis) for Spi-B and PU.1 motifs that show an absolute Log2 fold change > 1 and a p-value < 0.1, as calculated using the diffTF package for all Spi-B and PU.1 motif locations, (motif defined in the HOCOMOCO database). Data shown is the 9 (trisomy 12 CLL) versus 42 (non-trisomy 12 CLL) comparison based on the data from Rendeiro et al. 2016^3^.
...
## Appendix Figure S18. Genetic predictors of drug-stimulus interactions.
Based on the drug-stimulus viability data of 137 CLL patient samples with complete genetic annotation,  we fitted a sample specific linear model. Heatmap depicting overview of genetic predictors of drug - stimulus interactions (each row represents the coefficients from fitting a single multivariate model). Stimuli are shown on left, and corresponding drugs on right. Drugs, stimuli and genetic alterations are alphabetically sorted. Coloured fields indicate that the interaction coefficient for given drug and stimulus is modulated by corresponding genetic feature. Positive coefficients are shown in red, indicating the interaction coefficient is more positive for given drug and stimulus combination if the feature is present.
...
## Appendix Figure S19. Genetic predictors of the interaction between ibrutinib and IL4.
Predictor profile depicting genetic features that modulate the size of the $\beta_{int}$  for ibrutinib and IL4. To generate the predictor profile, linear model in Equation 1 was fitted in a sample-specific manner, to calculate drug-stimulus $\beta_{int}$ s for each patient sample (n=137). Ranked patient-specific $\beta_{int}$  values are shown in lower scatter plot. Associations between the size of the $\beta_{int}$  and genetic features were identified using multivariate regression with L1 (lasso) regularisation, with gene mutations and IGHV status as features, and selecting coefficients that were chosen in >90% of bootstrapped model fits. The horizontal bars  on left show the size of fitted coefficients assigned to genetic features. Matrix above scatter plot indicates patient mutation status for the selected genetic features. Matrix fields correspond to points in scatter plot (i.e. patient data is aligned), to indicate how the size of the $\beta_{int}$  varies with selected genetic feature.
...
IbrStat6iComb %>%
  mutate(trisomy12=as.character(trisomy12),
         IGHV.status=factor(IGHV.status, levels = c("U","M"), labels = c("unmutated", "mutated"))) %>% 
...
## Appendix Figure S20. STAT6 dependency of IL4 signaling.
...
1. Care, M.A., Cocco, M., Laye, J.P., et al. (2014) SPIB and BATF provide alternate determinants of IRF4 occupancy in diffuse large B-cell lymphoma linked to disease heterogeneity. Nucleic Acids Res 42: 7591–7610

2. Lu, J., Cannizzaro, E., Meier-Abt, F. et al. Multi-omics reveals clinically relevant proliferative drive associated with mTOR-MYC-OXPHOS activity in chronic lymphocytic leukemia. Nat Cancer 2, 853–864 (2021).


3. Rendeiro, A., Schmidl, C., Strefford, J. et al. Chromatin accessibility maps of chronic lymphocytic leukaemia identify subtype-specific epigenome signatures and transcription regulatory networks. Nat Commun 7, 11938 (2016).

4. Beekman, R., Chapaprieta, V., Russiñol, N. et al. The reference epigenome and regulatory chromatin landscape of chronic lymphocytic leukemia. Nat Med 24, 868–880 (2018).
