...
patMeta_tmp %>%
  filter(PatientID%in%unique(patMeta_tmp$PatientID)[1:80], Genetic_Alteration%in%c("del13q" ,"IGHV.status","TP53"   , "NOTCH1","trisomy12"  ) ) %>% 
  ggplot(aes(x=PatientID, y=Genetic_Alteration))+
  geom_tile(aes(fill = alteration_value), colour = NA, width=1)+
  geom_label(aes(y=Genetic_Alteration, label=Genetic_Alteration), x=3, hjust=0, size=8)+
  theme_temp+
  scale_fill_manual(values = c("0"=Mutant[1], "1"=Mutant[2], "M"=IGHV[1], "U"=IGHV[2],  "unknown"=na_color), na.value=na_color, labels = c("0"="wildtype", "1"="mutated", "M"="IGHV mutated", "U"="IGHV unmutated", "HP"="High programmed", "IP"="Intermediate programmed", "LP"="Low programmmed", "unknown"="N/A"))+
...
Natural logarithm of the relative viability after treatment BAFF, BAY-11-7085 or both, faceted by cluster. P-values from paired, two-sided Student's t-test are shown (C1: n=77; C2:n=27; C3: n=38; C4: n=50).
...
ggplot( aes(x=log2FoldChange, y=-log10(padj))) +
      annotate("rect", xmin = 0.0, xmax = Inf, ymin = -Inf, ymax = Inf, fill = palreds[1], alpha = 0.2)+
      annotate("rect", xmin = 0.0, xmax = -Inf, ymin = -Inf, ymax = Inf,alpha = 0.2, fill = palblues[1])+
      geom_point(aes(alpha=0.4, colour = ifelse(-log10(padj)>sgncuoff,"black","grey")), size=3) +
      theme(legend.position = "none") +
      geom_label_repel(aes(label=ifelse(-log10(padj)>sgncuoff&abs(log2FoldChange)>3,as.character(symbol),''), colour=ifelse(log2FoldChange>0, palreds[2], palblues[4])), fill="#FDFDFD", size=3.5, max.overlaps=30)+
      scale_colour_manual(values=c(palblues[2], palreds[4], "black", "grey"))+
      t2 +
      labs(title="Cluster 3 versus 4", x = "Log2FC", y = "-log10(adj p-val)", color = "Significance")+
      guides(color="none", alpha="none")+
...
plotTab <- diffTF_large %>%  filter(!is.na(pvalueAdj))
...
sgncuoff<-3
 ...
 ggplot(data = plotTab,
  ...
  geom_point(data=filter(plotTab, -log10(pvalueAdj) > 1.4), aes(alpha=0.4, colour = ifelse(weighted_meanDifference>0, palreds[1], palblues[1])),size=4.5) +
   
  geom_point(data=filter(plotTab, !(-log10(pvalueAdj) > 1.4)), aes(alpha=0.4), colour = "grey",size=2.5) +
  
  geom_label_repel(aes(label=ifelse(-log10(pvalueAdj) > 1.4&abs(weighted_meanDifference)>0.12,
                                    as.character(TF),''),
                       colour=ifelse(weighted_meanDifference>0,
                                     palreds[1],
                                     palblues[1])),
  ...
  scale_colour_manual(values=c(palblues[3], palreds[8], "black", "grey")) +
  t2 +
    annotate(geom="text", x=.1,y=.2, label="tri12", size=7, )+
    annotate(geom="text", x=-.1,y=.2, label="wt", size=7,)+
  ggtitle("ATAC seq: Tri12 v WT") +
  xlab("Change in inferred TF activity") +
  guides(color="none", alpha="none") +
  scale_y_continuous(expression("-log(adjusted  "* italic(p)*"-value)"))+
...
```{r , fig.path=plotDir, dev=c("cairo_pdf"), fig.height = 2, fig.width = 2, warning = FALSE, eval=FALSE}
