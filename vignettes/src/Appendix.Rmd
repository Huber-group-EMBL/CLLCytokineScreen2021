...
patMeta %>%
  mutate(IGHV.status=factor(IGHV.status, levels = c("U","M"), labels = c("unmutated", "mutated")),
         treatment=factor(treatment, levels = c(1,0), labels = c("Yes", "No")),
         Methylation_Cluster=factor(Methylation_Cluster, levels = c("LP","IP","HP"), labels = c("Low Programmed", "Intermediate Programmed", "High Programmed")),
...
List of patient samples and selected characteristics. For a full list of characteristics see [online vignette](https://github.com/Huber-group-EMBL/CLLCytokineScreen2021).NA=Not available.
...
```
## Appendix Table 2. List of genetic features tested for their association with stimulus response in the univariate model of Figure 3A.
...
## Appendix Table 3. Survival analysis of microenvironmental response clusters comparing C3 to C1, C2 and C4.
...
## Appendix Table 4. Survival analysis of microenvironmental response clusters comparing C3 to C1 and C2 combined.
...
## Appendix Table 5. Multivariate survival analysis of response clusters.
...
## Appendix Table 6. Over-representation tests of immune signalling pathways (and control pathways) amongst Spi-B gene targets, based on primary CLL ATACseq data.
...
```
## Appendix Table 7. Over-representation tests of immune signalling pathways (and control pathways) amongst Spi-B gene targets, based on DLBCL ChIPseq data from Care et al. 2014^1^. 
...
## Appendix Figure 2. Response to stimuli stratified by IGHV status.
...
## Appendix Figure 3. Stimulus response stratified by clusters.
...
## Appendix Figure 5. The relationship between CLL Proliferative Drive (as defined by Lu et al. (2021)^2^) and microenvironmental response.
...
## Appendix Figure 6. DNA Methylation profile of Clusters defined by response to microenvironmental stimuli
...
```{r, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height= 14, fig.width = 12, warning=F, message=F, cache=FALSE}
...
\newpage
## Appendix Figure 7. RNA-Sequencing of matched samples indicates differential gene expression between  Cluster 3 and Cluster 4.
...
```{r, fig.path=plotDir, dev=c("png", "cairo_pdf"), fig.height= 7, fig.width = 12, warning=F, message=F, cache=FALSE}
...
## Appendix Figure 8. RNA-Sequencing of matched samples indicates differential gene expression between Cluster 1 and 2.
...
## Appendix Figure 9. Drug response between clusters.
...
## Appendix Figure 10. Drug response by clusters.
...
## Appendix Figure 11. Effect of BAY-11-7085 treatment on BAFF response
...
## Appendix Figure 12. Correlation of stimulus response and receptor expression.
...
## Appendix Figure 13. Predictor profiles to represent genetic feature-stimulus associations.
...
ggplot(aes(start/10^6, l2FC))+
  ...
  ggtitle("Change in inferred TF activity for Spi-B motifs by Chromosome")+
  t2 + 
...
## Appendix Figure 17. Chromosomal locations of Spi-B motifs that show differential accessibility between trisomy 12 and non-trisomy 12 CLL. 
Log2 fold changes are plotted (y axis) for Spi-B motifs that show an absolute Log2 fold change > 1 and a p-value < 0.1, as calculated using the diffTF package for all Spi-B motif locations, (motif defined in the HOCOMOCO database). Data shown is the 9 (trisomy 12 CLL) versus 42 (non-trisomy 12 CLL) comparison based on the data from Rendeiro et al. 2016^3^.
...
load("C:/Users/Peter Bruch/Documents/GitHub/CLL_Proteomics/multiomics_MAE.RData")

# metadata(multiomics_MAE)$gene_symbol_mapping %>%  filter(hgnc_symbol=="SPI1")

# metadata(multiomics_MAE)$gene_symbol_mapping %>%  filter(hgnc_symbol=="SPIB")

##### SPI1
plot_Tab<-as_tibble(wideFormat(multiomics_MAE["SPI1",,"proteomics"] )) %>% 
left_join(select(as_tibble(wideFormat(multiomics_MAE[,,"RNAseq_norm"] )),primary, RNAseq_norm_ENSG00000066336), by="primary") %>%
left_join(select(as_tibble(wideFormat(multiomics_MAE[,,"chrom_abber"])), primary, chrom_abber_trisomy12), by="primary") %>% 
  mutate(chrom_abber_trisomy12=as.factor(chrom_abber_trisomy12))
  
p1<-filter(plot_Tab, !is.na(chrom_abber_trisomy12), !is.na(proteomics_SPI1)) %>% 
       ggplot(aes(chrom_abber_trisomy12, proteomics_SPI1, fill=chrom_abber_trisomy12, group=chrom_abber_trisomy12)) +
       geom_boxplot() + 
        geom_beeswarm() + 
       t2 +
       ylab(paste("Protein abundance SPI1")) + 
       xlab("trisomy 12") + 
       scale_fill_manual(labels=c("no", "yes"), values = c(palreds[8],palblues[1])) +
...
p2<-filter(plot_Tab, !is.na(chrom_abber_trisomy12), !is.na(RNAseq_norm_ENSG00000066336)) %>% 
       ggplot(aes(chrom_abber_trisomy12, RNAseq_norm_ENSG00000066336, fill=chrom_abber_trisomy12, group=chrom_abber_trisomy12)) +
       geom_boxplot() + 
        geom_beeswarm() + 
       t2 +
       ylab(paste("Gene expression SPI1")) + 
       xlab("trisomy 12") + 
       scale_fill_manual(labels=c("no", "yes"), values = c( palreds[5],palblues[4])) +
       theme( text = element_text(size=15)) +
       stat_compare_means(method = "t.test", 
                          aes(label = paste0("p = ", ..p.format..)), 
                          label.x = 1.5, hjust=0.5,
...
######### SPIB
plot_Tab<-select(as_tibble(wideFormat(multiomics_MAE[,,"RNAseq_norm"] )),primary, RNAseq_norm_ENSG00000269404) %>%
left_join(select(as_tibble(wideFormat(multiomics_MAE[,,"chrom_abber"])), primary, chrom_abber_trisomy12), by="primary") %>% 
  mutate(chrom_abber_trisomy12=as.factor(chrom_abber_trisomy12))
  

p3<-filter(plot_Tab, !is.na(chrom_abber_trisomy12), !is.na(RNAseq_norm_ENSG00000269404)) %>% 
       ggplot(aes(chrom_abber_trisomy12, RNAseq_norm_ENSG00000269404, fill=chrom_abber_trisomy12, group=chrom_abber_trisomy12)) +
       geom_boxplot() + 
        geom_beeswarm() + 
       t2 +
       ylab(paste("Gene expression SPIB")) + 
       xlab("trisomy 12") + 
       scale_fill_manual(labels=c("no", "yes"), values = c( palreds[5],palblues[4])) +
       theme( text = element_text(size=15)) +
       stat_compare_means(method = "t.test", 
