...
The experiment in Appendix Fig. 11, was performed as detailed in the main methods (Sample preparation and drug-stimulation profiling), but with the following adjustments. 16 independent CLL PBMC samples were used and luminescence was read out using a Perkin Elmer EnSight.
...
```{r Appendix Table 1 Pat Characteristics}
...
```
**Appendix Table 1. Patient samples included in the study.** List of patient samples and selected characteristics. For a full list of characteristics see [online vignette](https://github.com/Huber-group-EMBL/CLLCytokineScreen2021).
\newpage
...
## remove unused columns from metadata
  ...
  pivot_wider(names_from = alt_value, values_from = n) %>% 
   ...
   rename(Genetic_alteration=Genetic_alt, 'Wildtype'="0", 'Mutated'="1", 'Not available'='NA', "IGHV Mutated"="M", "IGHV Unmutated"="U")%>% 
...
**Appendix Table XX. List of genetic alterations tested for their association with stimuli response in the univariate model of Figure 3A** Only genetic alterations with at least 3 positive cases were considered. 
\newpage
...
**Appendix Table 2. Multivariate survival analysis of response clusters.** Multivariate Cox proportional hazards model of stimuli response clusters and genetic subgroups of disease progression using TTT and C3 as reference.
\newpage
...
<!-- # -->
<!-- # #Add meta data to heatmap clusters -->
<!-- # clusters.surv <- left_join(clusters.surv, CLL_PD, by="PatientID") -->
<!-- # -->
<!-- # #run coxphmodel -->
<!-- # c <- -->
<!-- #   clusters.surv %>% -->
<!-- #   mutate(Cluster=factor(Cluster)) %>% -->
<!-- #   coxph(Surv( TTT, treatedAfter) ~  factor2ind(Cluster, "3")+ CLL_PD,  data=.) -->
<!-- # -->
<!-- # #tidy results, exponentiate coefficients and estimate 95% confidence intervals -->
<!-- # cox_output<- tidy(c, conf.int = TRUE,  exponentiate = TRUE) %>% -->
<!-- #   dplyr::select(term, estimate, p.value, conf.low, conf.high) -->
...
```
**Appendix Table 3. Survival analysis of microenvironmental response clusters comparing Cluster 3 to the other Clusters.** 
...
```
**Appendix Figure 1. Genetic profiles of screened patient samples.**  Selected genetic alterations on y-axis and screened patient samples on x-axis.
\newpage
...
```{r  Cyokine viability beeswarms, fig.height=8, fig.width=16}
...
**Appendix Figure 2. Response to stimuli by IGHV status.** Viability after 48h incubation with microenvironmental stimuli. Log transformed viabilities, normalized to DMSO solvent controls, stratified by IGHV status.  BH-adjusted p-values are shown from one-sample t-tests of all patient samples. (p<0.00001=&#42;&#42;&#42;&#42;&#42;, p<0.0001=&#42;&#42;&#42;&#42;, p>0.05=ns) 
...
**Appendix Figure 4. Stimuli response by clusters.** Log transformed viabilities after treatment with stimuli, faceted by cluster. BH-adjusted p-values from student's t-tests are shown.
...
**Appendix Figure 5. Time to first treatment and overall survival by clusters.** Kaplan-Meier Curves of time to first treatment with p-value from univariate Cox proportional hazards model between C3 and C4 **(A)** and overall survival with p-value from univariate Cox proportional hazards  model between C1&2 and C3&4 **(B)**. Median survival not reached.
...
```{r, Fig2D,  dev=c("png", "cairo_pdf"), fig.height=7, fig.width=5}
load("../../data/multinominal_regression_Clusters_Fig2D.RData" )
...
**Appendix Figure XX.  Multinomial regression with lasso penalisation to identify enrichment or depletion of genetic features within each cluster.** 
...
```{r, fig.height=13, fig.width=10.5}
...
**Appendix Figure XXX. CLL Proliferative Drive as defined by Lu et al., 2021^5^** Beeswarm Plot of Proliferative drive in the clusters defined by microenvironmental resonse. P-values are shown from student's t-tests. **(A)** Predictor profiles for stimuli response to represent CLL Proliferative Drive identified through multivariate analysis using Gaussian linear modelling with L1-penalty for all patient samples which were analysed **(B)**, for only IGHV mutated **(C)** and for only IGHV unmutated patient samples **(D)** Within the plots, the bar plots on left indicate size and sign of coefficients assigned to the named predictors. Positive coefficients indicate higher viability after stimulation if the feature is present. Scatter plots indicate log(viability) values, in order of magnitude, for each individual sample. Heatmaps show status for each of the genetic predictors for corresponding sample in scatter plot.  Exemplary scatter plots devided by IGHV mutation status shown for Resiquimod **(E)** and IL1ß **(F)**. CLL-PD on x-axis and log normalised viability after microenvironmental stimulation on y-axis. Person correlation coefficients are shown with p-values. 
...
```{r , dev=c("png", "cairo_pdf"), fig.width=14.5, fig.height=11, warning = FALSE}
...
**Appendix Figure XXX. Drug response between clusters.**  Heatmap showing the log normalised viability after drug treatment.  Rows represent drug treatments and columns represent primary CLL samples, annotated for their genetic background, sex and pretreatment status above. Red values indicate increased viability upon treatment, blue indicates decreased viability.
...
stat.test <- df %>%
  ...
  group_by(Drug) %>%
  rstatix::t_test(Log ~ Cluster, p.adjust.method="BH", comparisons=list(c(1,2),c(3,4)))%>%
  rstatix::add_y_position(step.increase=0.001)
...
**Appendix Figure XX. Drug response by clusters.** Log transformed viabilities after treatment with drugs, faceted by cluster. BH-adjusted p-values from student's t-tests are shown.
...
```
**Appendix Figure 6. RNA-Sequencing of matched samples indicates differential gene expression between  Cluster 3 and Cluster 4.** Volcano plot of differentially expressed genes between Cluster 3 and Cluster 4 **(A)**. X axis indicates log2 fold change values, calculated using the Deseq package, y axis gives corresponding -log10(adjusted p value). P values adjusted using BH method. Genes are labeled where p value < 0.05. Enrichment plots of selected pathways **(B-D)**. Gene set enrichment analysis (GSEA) was performed with the Hallmark gene sets from the GSEA Molecular Signatures Database. Wald statistic was used to rank the genes.  The green curve corresponds to the Enrichment Score curve, which is the running sum of the weighted enrichment score obtained from GSEA software.

\newpage

```{r, fig.height= 7, fig.width = 12, warning=F, message=F, cache=FALSE}
...
**Appendix Figure XX. COmparison of gene expression between  Cluster 1 and Cluster 2.** Volcano plot of differentially expressed genes between Cluster 1 and Cluster 2. X axis indicates log2 fold change values, calculated using the Deseq package, y axis gives corresponding -log10(adjusted p value). P values adjusted using BH method. Genes are labeled where p value < 0.05.
...
**Appendix Figure XX. ** Predictor profiles to represent gene - stimulus associations for stimuli which are not shown in Figure 3B but have predictors >0.02 assigned by multivariate analysis using Gaussian linear modelling with L1-penalty. Within the plots, the bar plots on left indicate size and sign of coefficients assigned to the named predictors. Positive coefficients indicate higher viability after stimulation if the feature is present. Scatter plots indicate log(viability) values, in order of magnitude, for each individual sample. Heatmaps show status for each of the genetic predictors for corresponding sample in scatter plot. 

\newpage



```{r S8 receptor expression, fig.width=10, fig.height=7, warning=F}
...
```
**Appendix Figure 8. BCR inhibition by ibrutinib counters the protective effect of TLR stimulation in all genetic subgroups of IGHV and trisomy 12.** Log normalized viability after treatment with resiquimod with and without ibrutinib. Faceted by IGHV mutation status and trisomy 12.

\newpage

```{r S9 tri12atac_2v2, fig.height = 6, fig.width = 10}
...
**Appendix Figure 10. ATACseq comparing trisomy 12 and non-trisomy 12 CLL samples, without including TF binding sites on chromosome 12.** Volcano plot shows change in inferred TF activity (x axis) against adjusted p-values (y axis) for  trisomy 12 (n = 2) versus non-trisomy 12 samples (n = 2). Inferred TF activity calculated using the diffTF package, and measured as weighted mean difference.  p-values are obtained through diffTF in analytic mode and adjusted by the Benjamini-Hochberg procedure. TFs are labelled where absolute weighted mean difference > 0.15, and adjusted p-value < 0.01.


\newpage
```{R, S11, fig.width=15, fig.height=12, message=FALSE }
...
#get results with smallest p values
resTab <- dplyr::filter(resTab, pvalue_SPIB==0)

#filter for smallest p values and rank by number of targets = show this is still important 
resTab_ranked <- top_n(resTab, 20, count_SPIB)

resTab_ranked <- resTab_ranked[order(-resTab_ranked$count_SPIB),]

colnames(resTab_ranked) <- c("Pathway",
                      "Geneset\nDatabase" , 
                      "Geneset\nSize",  
...
S11 <- 
ggtexttable(resTab_ranked, rows = NULL, 
           theme = ttheme(
...
```
**Appendix Figure 11.  Over-representation tests of all KEGG and Reactome pathways in ChIPseq analysis of Spi-B binding in lymphoma cell lines. Results shown for pathways containing the most Spi-B gene targets (top 20). ** 
...
```
**Appendix Figure 12. Genetic predictors of drug - stimulus interactions.** Heatmap depicting overview of genetic predictors of drug - stimulus interactions (each row represents the coefficients from fitting a single multivariate model). Stimuli are shown on left, and corresponding drugs on right. Drugs, stimuli and genetic alterations are alphabetically sorted. Coloured fields indicate that the $\beta_{int}$ for given drug and stimulus is modulated by corresponding genetic feature. Positive coefficients are shown in red, indicating $\beta_{int}$ is more positive for given drug and stimulus combination if the feature is present.
...
p1
```
**Appendix Figure 14. STAT6 dependency of IL4 signaling.** The effects observed with IL4 stimulation are dependent on STAT6 activation. Addition of the STAT6 inhibitor AS1517499 could revoke the effect on baseline viability as well as drug induced toxicity.  Log transformed viability after treatment with IL4 in combination with ibrutinib, the STAT6 inhibitor AS1517499, and both.
...
5. Lu, J., Cannizzaro, E., Meier-Abt, F. et al. Multi-omics reveals clinically relevant proliferative drive associated with mTOR-MYC-OXPHOS activity in chronic lymphocytic leukemia. Nat Cancer 2, 853–864 (2021).
