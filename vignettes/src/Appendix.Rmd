...
<!-- # writexl::write_xlsx(Drug_table, "../../inst/extdata/Drugs.xlsx") -->


<!-- ``` -->
<!-- **Appendix Table 1. Drugs contained in the study.** List of drugs, along with their main target, supplier and concentration. All drugs were dissolved in Dimethyl Sulfoxide (DMSO) and stored at –20°C. -->
<!-- \newpage -->



<!-- ```{r Appendix Table 2, dev="cairo_pdf"} -->
...
<!--   kable() %>% -->
<!--   row_spec(0,bold=TRUE) %>% -->
<!--   kable_styling(latex_options = "striped", font_size=8, html_font = "arial") -->


<!-- # writexl::write_xlsx(Cytokine_table, "../../inst/extdata/Cytokines.xlsx") -->
<!-- ``` -->
<!-- **Appendix Table 2. Stimuli contained in the study.** List of microenvironmental stimuli, supplier, concentration and main pathway. -->

<!-- \elandscape -->

```{r Appendix Table 3 Pat Characteristics}
...
```
**Appendix Table 3. Patient samples included in the study.** List of patient samples and selected characteristics. For a full list of characteristics see [online vignette](https://github.com/Huber-group-EMBL/CLLCytokineScreen2021).
\newpage

```{r Appendix Table 4 OS ~ MES response}
...
**Appendix Table 4. Multivariate survival analysis of response clusters.** Multivariate Cox proportional hazards model of stimuli response clusters and genetic subgroups of disease progression using TTT and C3 as reference.
\newpage
...
```
**Appendix Table 5. Survival analysis of microenvironmental response clusters comparing Cluster 3 to the other Clusters.** 
...
<!--   mutate(Cluster3vs1_2=factor(if_else(Cluster==3, "3", "1_2")))%>% -->
<!--     coxph(Surv( TTT, treatedAfter) ~  factor2ind(Cluster3vs1_2, "3"), data=.)  -->
...
<!-- **Appendix Figure 2. Correlation of responses to stimuli and drugs.** Pearson correlation of log transformed viability values normalised to untreated control. Correlation between drug treatments  **(A)** and microenvironmental stimuli **(B)**. -->
...
```
**Appendix Figure XXX. CLL Proliferative Drive as defined by Lu et al., 2021^5^** Beeswarm Plot of Proliferative drive in the clusters defined by microenvironmental resonse. P-values are shown from student's t-tests. 
...
#filter fo smallest p values and rank by number of targets = show this is still important 
...
```
**Appendix Figure 11.  Over-representation tests of all KEGG and Reactome pathways in ChIPseq analysis of Spi-B binding in lymphoma cell lines ** 
