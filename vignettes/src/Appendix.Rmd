...
patMeta_Cluster %>% 
  select(PatientID, Cluster, Methylation_Cluster) %>% 
  mutate(Methylation_Cluster=factor(Methylation_Cluster, levels = c("HP","IP", "LP"))) %>%
  
  ggplot()+
    ggmosaic::geom_mosaic(aes(x = ggmosaic::product(Cluster), fill=Methylation_Cluster), alpha=1)+
  ylab("Incidence of\n methylation profiles")+
  scale_fill_manual(name = 'Methylation Profile', values = Methylation_cluster)+
  t2
...
\newpage
```{R, S11, fig.width=15, fig.height=20, message=FALSE }
...
resTab_ranked <- top_n(resTab, 40, count_SPIB)
...
```
**Appendix Figure 11.  Over-representation tests of all KEGG and Reactome pathways in ChIPseq analysis of Spi-B binding in lymphoma cell lines. Results shown for pathways containing largest number of Spi-B targets (top 40). ** 
